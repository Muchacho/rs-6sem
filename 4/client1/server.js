...
module.exports = (db) => {
    ...
    app.put('/update', async (req, res) => //синхронизация с центральной БД
    {
        // console.log(req.body);
        logger.debug('restore from central data base');
        // let response = await axios.get(`http://${ipServ}:8005/get`);
    ...
    app.get('/sTime', async function (req, res) {
        let response = await axios.get(`http://${ipServ}:8005/getTime`);
        let mnt = moment();
    ...
    app.get('/sync', async (req, res) => // синхронизация одной строки (id:3,дата,clNum: 2)
    {
        try {
            ...
            let response = await axios.post(`http://${ipServ}:8005/resource/`, {"id": 3, "clNum": 2});
    ...
    app.post('/set', async (req, res) => {
        ...
        let t_data = await req.body.data.filter(data => data.clNum === 2 && data.clNumId < 6);

        for (let i = 0; i < t_data.length; i++) {
            let data = t_data[i];
            data.id = data.clNumId+510;
            delete data.clNumId;
            logger.debug('restore ' + data);
