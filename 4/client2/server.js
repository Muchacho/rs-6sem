...
module.exports = (db) => {
    ...
    app.get('/sync', async (req, res) => {
        try {
            let response = await axios.post(`http://${ipServ}:8005/resource/`, {"id": 3, "clNum": 1});
    ...
    app.post('/set', async (req, res) => {
        // await db.model.destroy({where: {}, truncate: true});
        let t_data = await req.body.data.filter(data => data.clNum === 1 && data.clNumId < 6);
        for (let i = 0; i < t_data.length; i++) {
            let data = t_data[i];
            data.id = data.clNumId+100;
            delete data.clNumId;
            logger.debug('restore ' + data);
            await db.model.create(data);
        }
