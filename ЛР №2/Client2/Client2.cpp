...
int main(int argc, char* argv[])
{
	...
	//serv.sin_addr.s_addr = inet_addr("172.20.10.5"); //адрес сервера
	if (argv[1]) {
		serv.sin_addr.s_addr = inet_addr(argv[1]); //адрес сервера
	}
	else {
	...
	try
	{
		...
		while (true)
		{
			...
			if (ibuf[0] == 'o')
			{
				cout << "Now open Client2" << endl;

				Sleep(5000);

				string file_name = "\\\\DESKTOP-9ETH8PH\\Users\\Public\\qwe\\output.txt";
				ofstream fout(file_name, ios::app);
				for (int i = 0; i < 5; i++)
				{
					cout << "Text text text" << endl;
					fout << "Client cl2";//твой пользователь имя
					fout << "\n";
				}
				fout << "\n";
		...
		stop = clock();
		cout << "Time for sendto and recvfrom: " << (double)(stop - start) << endl;
