...
CA InitCA(const char ipaddr[15], const char resource[20])//инициализация секции
{
	CA ca;
	memcpy(ca.ipaddr, ipaddr, sizeof(ipaddr));//копирует num байтов первого блока памяти, 
	//на который ссылается указатель srcptr , во второй блок памяти, на который ссылается указатель
...
bool EnterCA(CA& ca)//вход в секцию
{
...
bool CloseCA(CA& ca)//закрыть секциюы
{
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		do
		{
			...
			cout << "IP-address: " << inet_ntoa(serv.sin_addr) << endl;
